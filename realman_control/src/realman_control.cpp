...
void execute(const control_msgs::FollowJointTrajectoryGoalConstPtr& goal, Server* as) {
    ...
    /***********Start gubs 2021/9/16 修复Moveit在同一位姿重复规划执行导致realman_contorl异常停止的Bug***********/
                    ...
                    if (spline.loadData(time_from_start, p_joint4, point_num, 0, 0, cubicSpline::BoundType_First_Derivative))
                    {
                        ...
                        if (spline.loadData(time_from_start, p_joint5, point_num, 0, 0, cubicSpline::BoundType_First_Derivative))
                        {
                            ...
                            if (spline.loadData(time_from_start, p_joint6, point_num, 0, 0, cubicSpline::BoundType_First_Derivative))
                            {
                                ...
                                ROS_INFO("Now We get all trajectory num %d", time_from_start_.size());
    ...
    /***********End   gubs 2021/9/16 修复Moveit在同一位姿重复规划执行导致realman_contorl异常停止的Bug***********/
...
{
    ros::init(argc, argv, "realman_arm_control");
    ros::NodeHandle nh;
    ...
    target_pub = nh.advertise<realman_msgs::JointPos>("/realman_driver/JointPos", 300);
    pub_getArmStateTimerSwitch = nh.advertise<std_msgs::Bool>("/realman_driver/GetArmStateTimerSwitch", 200);
    pub_getArmJoint = nh.advertise<std_msgs::Empty>("/realman_driver/GetArmJoint_Cmd", 100);
    
    // 定义一个服务器
    Server server(nh, "realman_65b/follow_joint_trajectory", boost::bind(&execute, _1, &server), false);
    // 服务器开始运行
    server.start();
    ros::spin();
