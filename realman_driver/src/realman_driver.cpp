...
void LiftHeightCtr_Callback(const realman_msgs::Lift_Height msg)
{
...
void LiftSpeedCtr_Callback(const realman_msgs::Lift_Speed msg)
{
...
void MoveJ_Callback(const realman_msgs::MoveJ msg)
{
    int res = 0;
    int i = 0;
    byte speed;
...
void MoveL_Callback(const realman_msgs::MoveL msg)
{
    int res = 0;
    byte speed;
    POSE target;
...
void MoveC_Callback(const realman_msgs::MoveC msg)
{
    int res = 0;
    byte speed;
...
void JointTeach_Callback(const realman_msgs::Joint_Teach msg)
{
...
void PosTeach_Callback(const realman_msgs::Pos_Teach msg)
{
...
void OrtTeach_Callback(const realman_msgs::Ort_Teach msg)
{
...
void StopTeach_Callback(const realman_msgs::Stop_Teach msg)
{
 ...
 * *********************************************************************************/
void MoveJ_P_Callback(const realman_msgs::MoveJ_P msg)
{
    int res = 0;
    byte speed;
    POSE target;
...
void ChangeToolName_Callback(const realman_msgs::ChangeTool_Name msg)
{
    int res = 0;
...
void ChangeWorkFrame_Callback(const realman_msgs::ChangeWorkFrame_Name msg)
{
    int res = 0;
...
void GetArmState_Callback(const realman_msgs::GetArmState_Command msg)
{
    int res = 0;
 ...
 * *********************************************************************************/
//开始复合模式拖动示教
void StartMultiDragTeach_Callback(const realman_msgs::Start_Multi_Drag_Teach msg)
{
...
void StopDragTeach_Callback(const std_msgs::Empty msg)
{
 ...
 //力位混合控制
void SetForcePosition_Callback(const realman_msgs::Set_Force_Position msg)
{
    int res = 0;
    if((msg.mode > 0) && (msg.mode <4))
    {
...
//位姿透传力位混合补偿
void ForcePositionMovePose_Callback(const realman_msgs::Force_Position_Move_Pose msg)
{
    int res = 0;
    POSE target;
	target = Quater_To_Euler(msg.Pose);
...
//角度透传力位混合补偿
void ForcePositionMoveJoint_Callback(const realman_msgs::Force_Position_Move_Joint msg)
{
    int res = 0;
    float joint[6];
    ...
    if((msg.mode >= 0) && (msg.mode <2))
    {
        if((msg.sensor >= 0) && (msg.sensor <2))
        {
            if((msg.dir >= 0) && (msg.dir <6))
            {
                res = Force_Position_Move_Joint_Cmd(msg.mode, msg.sensor, msg.dir, msg.force, joint);
                if(res == 0)
                {
                    ROS_INFO("Force Position Move Joint success!\n");
                }
                else
                {
                    ROS_ERROR("Force Position Move Joint failed!\n");
                }
            }
            else
            {
                ROS_ERROR("Force Position Move Joint dir wrong!\n");
            }
        }
        else
        {
            ROS_ERROR("Force Position Move Joint sensor wrong!\n");
        } 
    }
    else
    {
        ROS_ERROR("Force Position Move Joint mode wrong!\n");
    }
}
...
void ManualSetForcePose_Callback(const realman_msgs::Manual_Set_Force_Pose msg)
{
...
void JointPos_Callback(const realman_msgs::JointPos msg)
{
    int res = 0;
    int i = 0;
    float joint[6];
...
void Arm_DO_Callback(const realman_msgs::Arm_Digital_Output msg)
{
    int res = 0;
...
void Arm_AO_Callback(const realman_msgs::Arm_Analog_Output msg)
{
    int res = 0;
    if((msg.num > 0) && (msg.num <5))
    {
...
void Tool_DO_Callback(const realman_msgs::Tool_Digital_Output msg)
{
    int res = 0;
...
void Tool_AO_Callback(const realman_msgs::Tool_Analog_Output msg)
{
    int res = 0;
...
void Gripper_Pick_Callback(realman_msgs::Gripper_Pick msg)
{
    int res = 0;
...
void Gripper_Set_Callback(realman_msgs::Gripper_Set msg)
{
    int res = 0;
...
void Stop_Callback(const realman_msgs::Stop msg)
{
    int res = 0;
...
void Joint_Enable_Callback(const realman_msgs::Joint_Enable msg)
{
    int res = 0;
...
void  IO_Update_Callback(const realman_msgs::IO_Update msg)
{
    int res = 0;
    //Arm IO Update
    if(msg.type == 1)
 ...
 * *********************************************************************************/
void TurtleCtr_Callback(const realman_msgs::Turtle_Driver msg)
{
...
void Movep_Fd_Callback(const realman_msgs::CartePos msg){
...
int main(int argc, char **argv)
{
    ...
    //msg
    realman_msgs::Arm_IO_State Arm_IO;
    realman_msgs::Tool_IO_State Tool_IO;
    realman_msgs::Plan_State Plan;
    realman_msgs::ChangeTool_State ChangeTool_State;
    realman_msgs::ChangeWorkFrame_State ChangeWorkFrame_State;
    realman_msgs::Arm_Current_State Arm_State;
    realman_msgs::Force_Position_State Force_Position_State;
    realman_msgs::Six_Force Six_Force;
    std_msgs::Bool state;
    ...
    //publisher
    ChangeTool_Name = nh_.advertise<realman_msgs::ChangeTool_State>("/rm_driver/ChangeTool_State", 1);
    ChangeWorkFrame_Name = nh_.advertise<realman_msgs::ChangeWorkFrame_State>("/rm_driver/ChangeWorkFrame_State", 1);
    ArmCurrentState = nh_.advertise<realman_msgs::Arm_Current_State>("/rm_driver/Arm_Current_State", 10);
    /***** ********************************END****************************************/
    ...
    * *********************************************************************************/
	...
	Sub_ForcePositionMoveJoint = nh_.subscribe("/rm_driver/ForcePositionMoveJoint_Cmd", 10, ForcePositionMoveJoint_Callback);
	...
	pub_GetSixForce = nh_.advertise<realman_msgs::Six_Force>("/rm_driver/GetSixForce", 100);
	...
	pub_Force_Position_State = nh_.advertise<realman_msgs::Force_Position_State>("/rm_driver/Force_Position_State", 10);
    ...
    Arm_IO_State = nh_.advertise<realman_msgs::Arm_IO_State>("/rm_driver/Arm_IO_State", 1);
    Tool_IO_State = nh_.advertise<realman_msgs::Tool_IO_State>("/rm_driver/Tool_IO_State", 1);
    Plan_State = nh_.advertise<realman_msgs::Plan_State>("/rm_driver/Plan_State", 1);
    ...
    Arm_Socket_Close();
    ROS_INFO("RealMan_Robot driver shut down!\n");
    ros::waitForShutdown();
}
